	...
	<div class="content">
		<?php if ($data['notapproved']) : ?>
			<table>
				...
				<tbody>
				<?php foreach($data['notapproved'] as $key => $notapproved): ?>
				<tr class="<?php echo ($key % 2) ? 'odd' : 'even'; ?>">
					<td class="column-title"><a href="<?php _e('module_url'); ?>event/edit/<?php echo $notapproved['agenda_ID']; ?>"><?php echo $notapproved['agenda_title']; ?></a></td>
					<<td class="column-longtext"><?php echo $notapproved['content']; ?></td>
					<td class="column-text tcenter"><?php echo $notapproved['author']; ?></td>
					<!--<td class="column-buttons"><a href="<?php _e('module_url'); ?>comment/approve/<?php echo $notapproved['agenda_ID'] . '/' . $notapproved['agenda_ID']; ?>">aprobar</a> <a href="<?php _e('module_url'); ?>event/delete/<?php echo $notapproved['agenda_ID']; ?>">borrar</a></td>-->
				</tr>
				<?php endforeach; ?>
				</tbody>
			</table>
		...
		<?php if ($data['all_comments']): ?>
			<table>
				...
				<tbody>
					...
					<tr class="<?php echo ($key % 2) ? 'odd' : 'even'; ?>">
						<td class="column-title"><a href="<?php _e('module_url'); ?>event/edit/<?php echo $comment['agenda_ID']; ?>"><?php echo $comment['agenda_title']; ?></a></td>
						<td class="column-longtext"><?php echo $comment['content']; ?></td>
						<td class="column-text tcenter"><?php echo $comment['author']; ?></td>
						<td class="column-buttons">
							<?php if ($comment['approved']) : ?>
								<a href="<?php _e('module_url'); ?>comment/desapprove/<?php echo $comment['comment_ID'] . '/' . $comment['agenda_ID']; ?>">desaprobar</a>
							<?php else: ?>
								<a href="<?php _e('module_url'); ?>comment/approve/<?php echo $comment['comment_ID'] . '/' . $comment['agenda_ID']; ?>">aprobar</a>
							<?php endif; ?>
