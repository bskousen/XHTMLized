		...
		<table>
			...
			<?php foreach($data['banners'] as $key => $row): ?>
				<tr class="<?php echo ($key % 2) ? 'odd' : 'even'; ?>">
					...
					<td class="column-text"><?php echo $row['link'] ?></td>
					<td class="column-text tcenter"><?php echo $row['position_name']; ?></td>
					<td class="column-date"><?php echo fecha($row['date_added']); ?></td>
					<td class="column-buttons"><a href="<?php _e('module_url'); ?>banner/edit/<?php echo $row['banner_ID']; ?>">editar</a> <a href="<?php _e('module_url'); ?>banner/delete/<?php echo $row['banner_ID']; ?>">borrar</a></td>
				</tr>
			<?php endforeach; ?>
			</tbody>
		</table>
