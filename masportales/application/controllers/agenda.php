...
class Agenda extends Front_Controller {
	...
	public function _dia($date)
	{
		...
		$this->registry->set_meta('keywords', $this->registry->site('event_keywords'));
		$this->registry->set_meta('description', $this->registry->site('event_description'));
//echo "RES"; var_dump($this->event_model->getEventsFromDate($date));
		return array(
	...
	public function _calendar()
	{
		...
		$event_res = array();
		//$json = "{";
		foreach ($this->event_model->events_calendar() as $key =>$event) {
			//var_dump($event['event_start']);
			//	$z = rand(1, 1000);
				if ($event['event_start'] != $event['event_finish']){
					//TODO:meter los dias interiores
					$event_start_timestamp = strtotime($event['event_start']);
					$event_finish_timestamp = strtotime($event['event_finish']);
					$now = $event_start_timestamp;
					//echo 'now '.$now;echo "<br/>";
					//echo 'finish '.$event_finish_timestamp.'<br>';
					$event_res[$x] = date('Y-m-d H:i:s', $event_start_timestamp);
					$x++;
					$next_day = strtotime("+1 day", $now);
					$event_res[$x] = date('Y-m-d H:i:s', $next_day);
					$x++;
					while($now < $event_finish_timestamp) {
						//echo "mas";
						$now = strtotime("+1 day", $now);
						$event_res[$x] = date('Y-m-d H:i:s', $now);
						$x++;
					}
					//while($now < $event_finish_timestamp) {
						//$next_day = strtotime("+1 day", $now);
						//$next_day_date = date('Y-m-d H:i:s', $next_day);
						//$event_res[$x] = $next_day_date;
					/*	$now = $next_day_date;
						$x++;
						echo "ENTRA";
						var_dump($now);
				...
				}elseif (!in_array($event['event_start'], $event_res)) {
					//$event_res[] = $event['event_start'];	
					$event_res[$x] = $event['event_start'];
					$x++;
				}elseif (!in_array($event['event_finish'], $event_res)) {
					//$event_res[] = $event['event_start'];	
					$event_res[$x] = $event['event_finish'];
					$x++;
				}
		};
//echo "AQUI";var_dump($event_res);
//echo "AQUI";var_dump($result);
	//	header('Content-type: application/json');
	//	header ('Access-Control-Allow-Origin: *');
		//echo "llega"; var_dump($result);
		//$arr = array ("freenode-1" => "2012-06-29 00:00:00","2" =>"2012-06-30 00:00:00","3" =>"2012-07-01 00:00:00","4" =>"2012-06-28 00:00:00");
//echo "ASI SI!";
		//$this->autoRender = false;
		$this->output
   ->set_content_type('application/json')
   ->set_output(json_encode($event_res));

   //{"freedate-1":"2012-06-28 00:00:00","freedate-2":"2012-06-30 00:00:00","freedate-3":"2012-07-01 00:00:00"}
